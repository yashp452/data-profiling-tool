                            ...
                            if len(summary_df) > 0:
                                ...
                                with col1:
                                    try:
                                        ...
                                        st.caption("Missing Percentage by Column and Sheet")
                                    except Exception as e:
                                        # Fallback: simple bar chart by column
                                        column_avg = summary_df.groupby('Column')['Missing_Percentage'].mean()
                                        st.bar_chart(column_avg, use_container_width=True)
                                        st.caption("Average Missing Percentage by Column")
                                ...
                                with col2:
                                    try:
                                        ...
                                        # Fallback: show data as table if chart fails
                                        st.write("Missing Count by Sheet:")
                                        sheet_summary = summary_df.groupby('Sheet').agg({
                                            'Missing_Count': 'sum',
                                            'Missing_Percentage': 'mean'
                                        }).round(2)
                                        st.dataframe(sheet_summary)
