...
def create_html_report(df, sheet_name):
    """Create a custom HTML report for missing values analysis"""
    total_rows = len(df)
    ...
    fig1 = px.bar(stats_df, x='Column', y='Missing %', 
                  title=f'Missing Values Percentage by Column - {sheet_name}',
                  labels={'Missing %': 'Missing Percentage (%)'})
    fig1.update_xaxis(tickangle=45)
    
    fig2 = px.bar(stats_df, x='Column', y='Missing Count',
                  title=f'Missing Values Count by Column - {sheet_name}')
    ...
    <html>
    <head>
        <title>Missing Values Analysis - {sheet_name}</title>
        <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
        <style>
            body {{ font-family: Arial, sans-serif; margin: 40px; }}
            .header {{ background: #f0f2f6; padding: 20px; border-radius: 10px; margin-bottom: 30px; }}
            ...
            .metric-value {{ font-size: 24px; font-weight: bold; color: #262730; }}
    ...
    <body>
        <div class="header">
            <h1>Missing Values Analysis Report</h1>
            ...
            <div class="metrics">
                ...
                <div class="metric">
                    <div class="metric-value">{stats_df["Missing Count"].sum():,}</div>
                    <div class="metric-label">Total Missing Values</div>
                </div>
                <div class="metric">
                    <div class="metric-value">{stats_df["Missing %"].mean():.1f}%</div>
        ...
        {stats_df.to_html(index=False, classes='table')}
    ...
    try:
        ...
        for sheet_name, columns_to_analyze in sheet_column_mapping.items():
            ...
            try:
                ...
                try:
                    html_content = create_html_report(df_selected, sheet_name)
                        ...
                        if summary_data:
                            ...
                            if len(summary_df) > 0:
                                ...
                                with col2:
                                    ...
                                    sheet_totals = summary_df.groupby('Sheet')['Missing_Count'].sum().reset_index()
                                    fig2 = px.bar(
                                        sheet_totals,
                                        x='Sheet',
                                        y='Missing_Count',
                                        title='Total Missing Values by Sheet'
                                    )
